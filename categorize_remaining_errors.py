...
def categorize_errors():
    ...
    for i, query in enumerate(queries):
        ...
        if "{{" in query and ("Expected latitude" in error_text or "Expected longitude" in error_text):
        ...
        elif any(phrase in error_text for phrase in ["Expected set name", "map_to_area", "convert"]):
            categories["set_operations"].append((i+1, query, errors))
            categorized = True
            
        # Complex filter issues
        elif any(phrase in error_text for phrase in ["Expected }", "if:", "version()", "user:"]):
